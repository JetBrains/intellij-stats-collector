...
class EventSerializeDeserializeTest {
    ...
    fun `up down pressed event`() {
        var event: LogEvent = UpPressedEvent(Fixtures.userId, "xx", listOf(1, 2, 3),
                Fixtures.lookupList, 2, System.currentTimeMillis())
        serializeDeserializeAndCheck(event)

        event = DownPressedEvent(Fixtures.userId, "xx", listOf(1, 2, 3),
                Fixtures.lookupList, 2, System.currentTimeMillis())
        serializeDeserializeAndCheck(event)
    }
    ...
    fun `up down pressed event no additional items`() {
        var event: LogEvent = UpPressedEvent(Fixtures.userId, "xx", emptyList(),
                emptyList(), 2, System.currentTimeMillis())
        serializeDeserializeAndCheck(event)

        event = DownPressedEvent(Fixtures.userId, "xx", emptyList(),
                emptyList(), 2, System.currentTimeMillis())
        serializeDeserializeAndCheck(event)
    }
    ...
    fun `item selected by typing event`() {
        val event = TypedSelectEvent(Fixtures.userId, "xx", Fixtures.lookupList.drop(1), 5,
                Fixtures.lookupList, Fixtures.history, System.currentTimeMillis())
        serializeDeserializeAndCheck(event)
    }
    
    @Test
    fun `explicit select event`() {
        var event: LogEvent = ExplicitSelectEvent(Fixtures.userId, "xx", Fixtures.lookupList.drop(1),
                10, 10, Fixtures.lookupList, Fixtures.history, System.currentTimeMillis())
        serializeDeserializeAndCheck(event)
        
        event = ExplicitSelectEvent(Fixtures.userId, "xx", Fixtures.lookupList.drop(1),
                2, 2, Fixtures.lookupList, Fixtures.history, System.currentTimeMillis())
        serializeDeserializeAndCheck(event)
    }
